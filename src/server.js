...
// TEMPORARY IMPORT ROUTE (Batch 1)
app.get('/api/dev/import-batch1', async (req, res) => {
  ...
  try {
    for (const trip of tripsData) {
      ...
      const exists = await Trip.findOne({ title: trip.title });
      if (!exists) {
        await Trip.create(trip);
      }
    }
    res.json({ success: true, message: 'Batch 1 imported successfully' });
  } catch (err) {
